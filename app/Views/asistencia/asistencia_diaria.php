...
<body>
                    ...
                    <div class="col-lg-12 col-md-12">
                            ...
                            <div class="" >
                                <div class="" id="" role="" aria-labelledby="">
                                    <div class="card-body">
                                        ...
                                        <div class="col-md-12">
                                            <div class="card-footer text-black bg-white text-center">
                                                SIMBOLOGÍA
                                            </div>
                                            <div class="card-group">
                                                ...
                                                <div class="card bg-success">
                                                    <div class="card-body text-center text-white">
                                                        <span class="display-4"><i class="fa fa-check"></i></span>
                                                        <div class="mt-2">
                                                            <h5 class="font-weight-light text-white">PRESENTE</h5>
                                                        </div>
                                                    </div>
                                                </div>
                                                <!-- Column -->
                                                ...
                                                <div class="card bg-danger">
                                                    <div class="card-body text-center text-white">
                                                        <span class="display-4"><i class="fa fa-user-times"></i></span>
                                                        <div class="mt-2">
                                                            <h5 class="font-weight-light text-white">AUSENTE</h5>
                                                        </div>
                                                    </div>
                                                </div>
                                                <!-- Column -->
                        ...
                        <div class="card">
                            ...
                            <ul class="nav nav-pills custom-pills" id="pills-tab" role="tablist">
                                <li class="nav-item">
                                    <a class="nav-link active" id="pills-profesores-tab" data-toggle="pill" href="#last-month" role="tab" aria-controls="pills-profile" aria-selected="false">Listado de Alumnos</a>
                                </li>
                            </ul>
                            <!-- Tabs -->
                                        ...
                                        <div class="table-responsive">
                                            <table name="tabla_profesores" id="tabla_profesores" class="table table-striped table-bordered display" style="width:100%">
                                                <button class="btn btn-danger waves-effect waves-light m-1 mb-3" type="button" data-toggle="modal"
                                                    data-target="#modal-atrasos-diaria"><span class="btn-label"><i class="fa fa-clock"></i></span>
                                                    Atrasos</button>
    ...
    <script>
        ...
        var dataAsistencias = []
        <?php foreach($alumnos as $rp) { ?>
            asistenciaAlumno = '';
            switch('<?= $rp->asistencia ?>') {
                case "0":
                    asistenciaAlumno = '<span id="alum'+'<?= $rp->oid ?>'+'" class="badge py-1 badge-secondary">Sin Asistencia</span>';
                    break;
                case "1":
                    asistenciaAlumno = '<span id="alum'+'<?= $rp->oid ?>'+'" class="badge py-1 badge-success">Presente</span>';
                    break;
                case "2":
                    asistenciaAlumno = '<span id="alum'+'<?= $rp->oid ?>'+'" class="badge py-1 badge-danger">Ausente</span>';
                    break;
                }
            dataAsistencias.push(['<?= $rp->oid ?>','<?= $rp->rut ?>','<?= $rp->nombres ?>','<?= $rp->apellido_paterno ?>','<?= $rp->apellido_materno ?>',
                        asistenciaAlumno,
                        '<button onclick="addAsistenciaAlumno('+'<?= $rp->oid ?>'+",'presente'"+')" type="button" class="btn waves-effect waves-light btn-rounded btn-outline-success btn-sm mr-1" aria-label="""><i class="fa fa-check"></i></button>'+
                        '<button onclick="addAsistenciaAlumno('+'<?= $rp->oid ?>'+",'ausente'"+')" type="button" class="btn waves-effect waves-light btn-rounded btn-outline-danger btn-sm" aria-label="""><i class="fa fa-user-times"></i></button>'
                    ]);
        <?php } ?>
    ...
    <script>
        function addAsistenciaAlumno(id, estado){
            var url = "<?php echo base_url('public/Cursos/add_asistencia_alumno/'); ?>";
            $.post(url, {id_asistencia: <?= $id_asistencia ?>, id_alumno: id, estado: estado}, function(data, status){
                if (status){
                    if(data){
                        if(estado == 'presente'){
                            document.getElementById("alum"+id).className = 'badge py-1 badge-success';
                            document.getElementById("alum"+id).innerHTML = 'Presente';
                        }else{
                            document.getElementById("alum"+id).className = 'badge py-1 badge-danger';
                            document.getElementById("alum"+id).innerHTML = 'Ausente';
                        }
                        actualizarAlumnosAtrasos();
                    }
                }else{
                    console.log("ERROR");
                }
        ...
        function actualizarAlumnosAtrasos(){
            var url = "<?php echo base_url('public/Cursos/getAlumnosAtrasos/'); ?>";
                ...
                if (status){
                    ...
                    if(data != []){
                        ...
                        for(var i = 0; i < data.length; i++){
                            if (data[i].asistencia == 2)
                                datos += "<option value='"+data[i].oid_usuario+"'>"+data[i].rut+" "+data[i].nombres+" "+data[i].apellidos+" "+"</option>";
                        }
                        $("#alumno_atraso2").html(datos);
                    }
                }else{
                    console.log("ERROR");
                }
