...
<body>
    ...
    <script>
        ...
        <?php foreach($resultado_busqueda as $rb) { ?>
            dataSet.push(['<?= $rb->oid ?>', '<?= $rb->oid ?>', '<?= $rb->nombre ?>', 
                        '<?= ($rb->estatus ? "No" : "Si") ?>',
                        '<a href="<?php echo site_url('carrera/editar/'.$rb->oid); ?>" type="button" class="btn waves-effect waves-light btn-rounded btn-outline-success btn-sm" aria-label=""">Editar</a>']);
            // console.log(dataSet)
        <?php } ?>
