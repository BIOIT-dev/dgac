...
<body>
    ...
    <script>
        ...
        <?php foreach($resultado_busqueda as $rb) { ?>
            dataSet.push(["<?=$rb->nombre ?>", '<?= $rb->gcnombre ?>', 
                        "<?= $rb->numUsers ?>", "<?=$rb->autoincorporacion ? "Si" : "No"?>", '<?=$rb->inactivo ? "No" : "Si"?>',
                        '<a href="<?php echo site_url('ayudaComunidad/editar/'.$rb->oid); ?>" type="button" class="btn waves-effect waves-light btn-rounded btn-outline-success btn-sm" aria-label=""">Editar</a>']);
            // console.log(dataSet)
        <?php } ?>
