    ...
    <script>
        ...
        <?php foreach($r_alumnos as $ralumnos) { ?>
            dataAlumnos.push(['<?= $ralumnos->oid ?>','<?= $ralumnos->rut ?>','<?= $ralumnos->nombres ?>','<?= $ralumnos->apellido_paterno ?>','<?= $ralumnos->apellido_materno ?>',
                        ...
                        '<a href="<?php echo site_url('progPresencial/editar_alumno/'.$profile_data_edit['oid'].'/'.$ralumnos->oid); ?>" type="button" class="btn waves-effect waves-light btn-rounded btn-outline-success btn-sm" aria-label=""">Estado</a>'+'<button onclick="cambiarVigencia('+'<?= $ralumnos->oid.','.$ralumnos->inactivo ?>'+')" type="button" class="btn waves-effect waves-light btn-rounded btn-outline-success btn-sm" aria-label=""">Vigencia</button>']);
        <?php } ?>
        $(document).ready(function() {
